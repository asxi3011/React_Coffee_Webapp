...
const DetailProduct = () => {
    ...
    const handleAddCart = () => {
        ...
        const arrayTrung = cart.filter(ele=>(ele._id===carts._id && ele.sizeValueSelected === carts.sizeValueSelected && ele.note===carts.note));
        if (arrayTrung.length>0){
            cart.map((ele,index)=>{
                if(ele._id === carts._id && ele.sizeValueSelected === carts.sizeValueSelected&& ele.note===carts.note){
                        ...
                        <div className="row">
                            <div className="col-6">
                                <div className="details-image-info-product">
                                    <img src={`https://sever-coffeehouse.herokuapp.com/uploads/${info.imageRepresent}`} alt="" />
                                </div>
                                ...
                                <p className="mt-4">{info.descriptionProduct}
                                </p>
                            </div>

                            <div className="col-6 ">
                                <h2 id="name_product" className="d-block price-size-show fw-bold"   >{info.nameProduct}</h2>
                                <div className="d-flex justify-content-between align-items-center">
                                    <div>
                                        <span id="" className="details-price-prodcut">{info.priceStandard.toLocaleString()}đ</span>
                                ...
                                {<ListSize sizes={info.Size} checked={checked} setChecked={setChecked}/>}

                                <button type="submit" className="btn btn-color-primary w-100 mt-4"
                                    id="btn_addToCart" onClick={handleAddCart} >
                                    {
                                        (count * (info.priceStandard + Number(checked.value))).toLocaleString()
                                    }đ - Thêm vào giỏ hàng
